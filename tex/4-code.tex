...
\section{BIGQUAKE}
In this Section we will discuss the BIGQUAKE submission, 
...
\begin{enumerate}
    \item $A$ generates a random $m \in \mathbb{F}^s_2$;
...
Sort $L$ in descending order of $T$\;
...
After finding the position of the errors, one needs to verify if the error $e'$ found is the correct one, and then recover the message $m$. In order to verify for correctness, one can check $e'$ by computing $\mathcal{H}(e) \oplus \mathcal{H}(e') \oplus m = m'$ and if $c_3$ is equal to $\mathcal{H}(m')$. As mentioned in Subsection~\ref{subsec:bigquake}, the ciphertext is composed by $c = (m\oplus\mathcal{H}(e), H\cdot e^T, \mathcal{H}(m))$ or $c = (c_1, c_2, c_3)$.