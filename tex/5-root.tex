...
Using this approach, we add one extra step to the algorithm. However, this permutation blurs the sensitive information of the algorithm, making the usage of Algorithm~\ref{alg:exhaustive_permuted} slightly harder for the attacker to acquire timing leakage. For this case, we want to avoid the addition of an \texttt{else} clausure adding the elements that are not a root in other l
