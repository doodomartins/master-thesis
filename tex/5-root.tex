...
\end{equation}
If one solves simultaneously the linear system in Equation~\ref{eq:example_1_6} then the results are the roots of the original polynomial given in Equation~\ref{eq:example_1}. From Equation~\ref{eq:example_1_5}, one observes that the solutions are $y=110$ and $y=011$. Furthermore, we can translate $110$ and $011$ to $\alpha + 1$ and $\alpha^2 + \alpha$. After all this steps, we can check if we got the correct roots by checking $f(\alpha + 1)$ and $f(\alpha^2 + \alpha)$

\begin{equation}\label{eq:example_1_6}
    f(\alpha + 1) = (\alpha + 1)^2 + (\alpha^2 + 1)(\alpha + 1) + 
\end{equation}
