...
This adaptation introduces an trace computation and slight decrease their performance but it is one of the fastest method to finding roots over a finite field~\cite{von2001factoring}. The main difference between Cantor-Zassenhaus method and the others methods presented in this chapter was the insertion of an randomness in the algorithm. The computation of the roots depends was given by the choice of an random element in $\mathbb{F}_{2^{m}}$, this random selection was used with an trace function and an gcd to find an nontrivial factorization of the original polynomial. 

Cantor-Zassenhaus is an probabilistic algorithm because this random behaviour, and the parameter $\epsilon$ was used to define the maximum number of iterations on the algorithm. Although it is a probabilistic algorithm, Ben-Or prove that it is  
...
\begin{algorithm}[ht]
 ...
 \KwResult{The set of roots of $p(x)$.}
    ...
    \Return{$Factors\;$}
 \caption{Successive Resultant Algorithm -- $SCA(p(x))$.}
  \label{alg:new}
\end{algorithm}
