...
For each algorithm presented, we can observe the same behaviour on execution time. Since we has the complexity of all algorithm in terms of the degree $t$ of the polynomial and the size of the field $n = 2^m$, we can note that the algorithm has an 
